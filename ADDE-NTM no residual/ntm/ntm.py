...
class NTM(nn.Module):
    ...
    def __init__(self, num_inputs, num_outputs, controller, memory, heads):
        ...
        self.fc = nn.Linear(self.controller_size + self.num_read_heads * self.M, num_outputs).cuda()
    ...
    def forward(self, x, prev_state):
        ...
        inp = torch.cat([x] + prev_reads, dim=1).cuda()
        ...
        # Generate Output
        inp2 = torch.cat([controller_outp] + reads, dim=1).cuda()
        o = torch.sigmoid(self.fc(inp2).cuda()).cuda()
