...
# def train(train_loader, model, optimizer, args):
    ...
    for i, (imgs, one_hots, labels, indicator) in enumerate(tqdm(train_loader)):
        ...
        # indicator_np = indicator.cpu().detach().numpy()
        # save_pkl('../pickle_files/indicator', indicator_np)
        ...
        ind = indicator.data.cpu().detach().numpy()
        mem_init = deepcopy(memory.Memory.weight.data.cpu().detach().numpy())
        residual_feat = memory(indicator)
        res_feat = residual_feat.data.cpu().detach().numpy()
        ...
        total_loss.backward()
        optimizer.step()
        mem_after = memory.Memory.weight.data.cpu().detach().numpy()
...
def eval(gallery_loader, query_loader, model, memory, args, file_root):
    model.eval()
    memory.eval()
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        avg_test_acc = eval(gallery_loader, query_loader, model, memory, args, file_root)
        # avg_test_acc = eval(gallery_loader, query_loader, model, args, file_root)
        ...
        if train_bool:
            ...
            if current_epoch % 10 == 0:
                torch.save(model.state_dict(), os.path.join(directory, "manip", "extractor_ckpt_%d.pkl" % (current_epoch)))
                torch.save(memory.state_dict(), os.path.join(directory, "manip", "memory_ckpt_%d.pkl" % (current_epoch)))
                print('Saved checkpoints at {dir}/manip/extractor_{epoch}.pkl, {dir}/manip/memory_{epoch}.pkl'.format(dir=directory,
                                                                                                          epoch=current_epoch))
            if avg_test_acc > previous_best_avg_test_acc:
                torch.save(model.state_dict(), os.path.join(directory, "manip", "extractor_best.pkl"))
                torch.save(memory.state_dict(), os.path.join(directory, "manip", "memory_best.pkl"))
                print('Best model in {dir}/manip/extractor_best.pkl and {dir}/manip/memory_best.pkl'.format(dir=directory))
                previous_best_avg_test_acc = avg_test_acc

            if avg_total_loss < previous_best_avg_total_loss:
                torch.save(model.state_dict(), os.path.join(directory, "manip", "extractor_best_loss.pkl"))
                torch.save(memory.state_dict(), os.path.join(directory, "manip", "memory_best_loss.pkl"))
                print('Best model loss in {dir}/manip/extractor_best_loss.pkl and {dir}/manip/memory_best_loss.pkl'.format(dir=directory))
