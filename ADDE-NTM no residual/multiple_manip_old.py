...
if __name__ == '__main__':
    ...
    print('Forward')
    for z in range(num_image_query):
        total_hit = 0
        ...
        with torch.no_grad():
            ...
            # for i, (operation, img, indicator, image_name) in enumerate(tqdm(query_loader)):
            #     if operation:
        ...
        # print('Top@{k} accuracy: {acc}'.format(k=k, acc=total_hit/number_of_manipulation))
        # f.write('Top@{k} accuracy: {acc} \n'.format(k=k, acc=total_hit/number_of_manipulation))
