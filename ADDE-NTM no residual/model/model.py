...
class NTM_extractor(nn.Module):
    ...
    def forward(self, indicator, imgs, labels, train_loader, optimizer, args, train):
        ...
        if train:
            ...
            input_memory = torch.zeros([indicator.shape[0], 12, 340]).cuda()
            for i in range(input_memory.shape[0]):
                for j in range(input_memory.shape[1]):
                    input_memory[i][j] = torch.reshape(feats['ref'][j][i], (1, input_memory.shape[2])).cuda()
        else:
            dis_feat, _ = self.model_feature(imgs)
            input_memory = torch.zeros([indicator.shape[0], 12, 340]).cuda()
            # dis_feat = torch.cat(dis_feat, 1).cuda()
            for i in range(input_memory.shape[0]):
                for j in range(input_memory.shape[1]):
        ...
        feat_manip = ntm_manip.manipulate_ntm(self, optimizer, indicator, train, input_memory)
        # feat_manip = feat_manip[0]
        ...
        if train:
            ...
            # feat_manip = torch.cat(dis_feat, 1) + feat_manip
            total_loss = -1
            feats = []
            cls_outs = []
            cls_outs_manip = []
