...
if __name__ == '__main__':
    ...
    if not training_model:
        ...
        init_weight_memory = np.load(ckpt_dir + "/manip/memory_" + training_model_path + ".npy")
        init_weight_memory = torch.from_numpy(init_weight_memory).cuda()
    ...
    with torch.no_grad():
        for i, (img, indicator) in enumerate(tqdm(query_loader)):
            ...
            len_indicator = indicator.shape[0]
            if len_indicator != batch_size:
                for k in range(len_indicator, batch_size):
                    new_row = torch.zeros(1, indicator.shape[1]).cuda()
                    indicator = torch.cat((indicator, new_row), 0)

            residual_feat = ntm_manip.manipulate_ntm(model_ntm_feature, False, indicator, False, None)

            if len_indicator != batch_size:
                residual_feat = residual_feat[0:len_indicator, :]
