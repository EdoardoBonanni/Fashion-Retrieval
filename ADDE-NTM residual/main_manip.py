...
def main():
    ...
    for epoch in range(epoch_num, num_epochs):
        ...
        if train_bool:
            ...
            memory = ntm_manip.mem_bias_state(model_ntm_feature)
            # store parameters
            if current_epoch % 5 == 0:
                ...
                print('Saved checkpoints at {dir}/manip/extractor_epoch_{epoch}.pkl, {dir}/manip/ntm_epoch_{epoch}.model'.format(dir=directory,
                                                                                                                       epoch=current_epoch))
                np.save(os.path.join(directory, "manip", 'memory_epoch_%d.npy' % (current_epoch)), memory)
                print('Saved indexed features at {dir}/manip/memory_epoch_{epoch}.npy'.format(dir=directory, epoch=current_epoch))
            ...
            if avg_test_acc > previous_best_avg_test_acc:
                ...
                print('Best model in {dir}/manip/extractor_best_acc_{epoch}.pkl and {dir}/manip/ntm_best_acc_{epoch}.model'.format(dir=directory,
                                                                                                                 epoch=current_epoch))
                np.save(os.path.join(directory, "manip", 'memory_best_acc_%d.npy' % (current_epoch)), memory)
                print('Saved indexed features at {dir}/manip/memory_best_acc_{epoch}.npy'.format(dir=directory, epoch=current_epoch))
            ...
            if avg_total_loss < previous_best_avg_total_loss:
                ...
                print('Best model loss in {dir}/manip/extractor_best_loss_{epoch}.pkl and {dir}/manip/ntm_best_loss_{epoch}.model'.format(dir=directory,
                                                                                                                        epoch=current_epoch))
                np.save(os.path.join(directory, "manip", 'memory_best_loss_%d.npy' % (current_epoch)), memory)
                print('Saved indexed features at {dir}/manip/memory_best_loss_{epoch}.npy'.format(dir=directory, epoch=current_epoch))
