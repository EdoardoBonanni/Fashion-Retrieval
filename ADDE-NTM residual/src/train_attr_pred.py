...
if __name__ == '__main__':
    ...
    ckpt_dir = "../models/Shopping100k/encoder"
    memory_dir = '../' + ckpt_dir + "initialized_memory_block/"
    batch_size = 256
    # batch_size = 64 # batch size iniziale (stessa val_acc con batch_size diverso)
    num_epochs = 100
    ...
    train_loader = data.DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=args.num_threads,
                                   drop_last=True)
    valid_loader = data.DataLoader(valid_data, batch_size=batch_size, shuffle=False, num_workers=args.num_threads,
                                   drop_last=False)
    ...
    for epoch in range(num_epochs):
        ...
        if train_bool:
            if (epoch + 100 + 1) % 5 == 0:
                torch.save(model.state_dict(), os.path.join(directory, "ckpt_%d.pkl" % (epoch + 100 + 1)))
                print('Saved checkpoints at {dir}/ckpt_{epoch}.pkl'.format(dir=directory, epoch=epoch+100+1))
            ...
            if avg_total_loss < previous_best_avg_total_loss:
                torch.save(model.state_dict(), os.path.join(directory, "extractor_best_loss.pkl"))
                print('Best model loss in {dir}/extractor_best_loss.pkl'.format(dir=directory))
